...
void Simulador::iniciarSimulacao()
{
    ...
    while (calcularVidaEquipe(1) > 0 && calcularVidaEquipe(2) > 0)
    {
        ...
        do
        {
            indiceEquipe1 = std::rand() % equipe1.size();
            indiceEquipe2 = std::rand() % equipe2.size();
        }

        while (equipe1[indiceEquipe1]->getVida() < 1 || equipe2[indiceEquipe2]->getVida() < 1);

        if (equipeQueAtaca == 1)
        {
            personagemAtacante = equipe1[indiceEquipe1];
            personagemDefesa = equipe2[indiceEquipe2];
        }

        if (equipeQueAtaca == 2)
        {
            personagemAtacante = equipe2[indiceEquipe2];
            personagemDefesa = equipe1[indiceEquipe1];
        }
